...
try {
  ...
  if (replay !== undefined) {
    ...
    console.log(`[issue-25] replay passed; seed=${seed} path=${replay}`);
  } else {
    ...
    if (JSON.stringify(expectedNames) !== JSON.stringify(actualNames)) fail("committed shard set is dirty; run pnpm generate:issue25");
    for (const name of expectedNames) {
      const [expected, actual] = await Promise.all([readFile(resolve(committed, name), "utf8"), readFile(resolve(temporary, name), "utf8")]);
      if (expected !== actual) fail(`${name} is dirty; run pnpm generate:issue25`);
