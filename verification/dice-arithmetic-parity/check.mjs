/* Blocking arithmetic/static parity and oracle-boundary gate for Dice arithmetic parity. */
...
const fail = (message) => { throw new Error(`[dice-arithmetic-parity] ${message}`); };
...
const temporary = await mkdtemp(resolve(tmpdir(), "drdice-arithmetic-parity-generated-"));
try {
  ...
  const prefix = "dice-arithmetic-parity-";
  ...
  equal(actualNames, committedNames, "Dice arithmetic parity generated fixture set is dirty");
...
console.log(`[dice-arithmetic-parity] ${ids.length} independent oracle-backed arithmetic/static cases and clean TypeScript 7 fixtures passed`);
