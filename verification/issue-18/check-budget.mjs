...
const generatedNames = (await readdir(generatedDirectory))
  .filter((name) => /^prng-issue18-(?:transitions-\d+|replay)\.d\.ts$/.test(name))
  .sort();
const expectedTransitionNames = Array.from(
  { length: 10 },
  (_, index) => `prng-issue18-transitions-${index}.d.ts`,
);
const expectedNames = [...expectedTransitionNames, "prng-issue18-replay.d.ts"];
if (JSON.stringify(generatedNames) !== JSON.stringify([...expectedNames].sort())) {
  throw new Error(`issue-18 budget artifacts differ; expected ${expectedNames.join(", ")}, got ${generatedNames.join(", ")}`);
}
const artifacts = [
  focusedQuery,
...
for (const checkers of [1, 4]) {
  for (const artifact of artifacts) {
    ...
    const run = () => {
      ...
      if (checked.status !== 0) throw new Error(`issue-18 ${label} query failed\n${output}`);
      return {
