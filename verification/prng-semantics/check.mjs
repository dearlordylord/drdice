...
const fail = (message) => {
  throw new Error(`[prng-semantics] ${message}`);
};
...
const checkPrivateBoundary = () => {
  ...
  assert(oraclePath.includes(`${path.sep}verification${path.sep}prng-semantics${path.sep}`), "oracle is outside the private verification boundary");
};
...
const main = () => {
  ...
  console.log(`[prng-semantics] oracle/golden checks passed (${path.relative(process.cwd(), goldenFile)})`);
};
