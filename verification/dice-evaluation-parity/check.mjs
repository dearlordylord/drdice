/* Blocking oracle, corpus, and exact-type gate for Dice evaluation parity. */
import { mkdtemp, readdir, readFile, rm } from "node:fs/promises";
import { tmpdir } from "node:os";
import { dirname, relative, resolve } from "node:path";
import { fileURLToPath } from "node:url";
import { spawnSync } from "node:child_process";
import { LIMITS, oracleEvaluate } from "../dice-semantics/oracle.mjs";
import { oracleStateFromWords } from "../prng-semantics/oracle.mjs";

const here = dirname(fileURLToPath(import.meta.url));
const root = resolve(here, "../..");
const generated = resolve(here, "generated");
const ids = JSON.parse(await readFile(resolve(here, "cases.json"), "utf8"));
const golden = JSON.parse(await readFile(resolve(root, "verification/dice-semantics/golden-vectors.json"), "utf8"));
const byId = new Map(golden.cases.map((vector) => [vector.id, vector]));
const fail = (message) => { throw new Error(`[dice-evaluation-parity] ${message}`); };
...
const names = (await readdir(generated)).filter((name) => /^dice-evaluation-parity-(?:\d{3}|side-\d{3})\.d\.ts$/.test(name)).sort();
const expectedNames = [
  ...ids.map((_, index) => `dice-evaluation-parity-${String(index).padStart(3, "0")}.d.ts`),
  ...Array.from({ length: LIMITS.supportedSideCount }, (_, index) => `dice-evaluation-parity-side-${String(index + 1).padStart(3, "0")}.d.ts`),
].sort();
equal(names, expectedNames, "Dice evaluation parity generated shard set is incomplete or dirty");

const temporary = await mkdtemp(resolve(tmpdir(), "drdice-evaluation-parity-generated-"));
try {
  ...
  equal(actualNames, names, "Dice evaluation parity generated shard set is not reproducible");
...
const oracleSource = await readFile(resolve(root, "verification/dice-semantics/oracle.mjs"), "utf8");
if (/(?:packages|src|dist)\//.test(oracleSource)) fail("Dice semantics oracle references production paths");
...
console.log(`[dice-evaluation-parity] ${vectors.length} oracle-backed full-evaluation cases, every side d1..d100, and ${names.length} exact TypeScript 7 fixtures passed`);
