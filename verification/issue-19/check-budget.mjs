...
const generatedNames = (await readdir(generatedDirectory)).filter(
  (name) => /^prng-issue19-(?:grid-\d{3}|special-\d{3})\.d\.ts$/.test(name),
).sort();
const expectedNames = [
  ...Array.from({ length: 500 }, (_, index) => `prng-issue19-grid-${String(index).padStart(3, "0")}.d.ts`),
  ...Array.from({ length: 3 }, (_, index) => `prng-issue19-special-${String(index).padStart(3, "0")}.d.ts`),
].sort();
if (JSON.stringify(generatedNames) !== JSON.stringify(expectedNames)) {
  throw new Error(`issue-19 budget artifacts differ; expected ${expectedNames.length}, got ${generatedNames.length}`);
}
...
if (version.status !== 0 || version.stdout.trim() !== "Version 7.0.2") {
  throw new Error(`issue-19 budget requires TypeScript 7.0.2, got ${version.stdout.trim()}\n${version.stderr}`);
}
...
for (const checkers of [1, 4]) {
  for (const artifact of artifacts) {
    ...
    const run = () => {
      ...
      if (checked.status !== 0) throw new Error(`issue-19 ${label} query failed\n${output}`);
      return {
