 ...
 *
 * Issue #17's branch predates the package scaffold, so this gate is tolerant
 * of absent package roots and reports the deferred check.  Once #16 supplies
...
const fail = (message) => {
  throw new Error(`[issue-17 boundary] ${message}`);
};
...
const checkPackage = (packageRoot) => {
  ...
  for (const file of files(packageRoot)) {
    ...
    if (relative.endsWith(".d.ts")) {
      ...
      assert(!/(verification\/issue-17|golden-vectors|oracle\.mjs|oracle\.js)/i.test(source), `${manifest.name ?? packageRoot} declaration imports the private oracle`);
    }
  }
...
if (present.length === 0) {
  console.log("[issue-17 boundary] package roots are not present yet; boundary check deferred to the #16 scaffold");
} else {
  ...
  console.log(`[issue-17 boundary] checked ${present.length} package roots; private oracle is not exposed`);
}
