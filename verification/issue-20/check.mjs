/*
 * Blocking issue #20 oracle/golden gate.
 *
...
const fail = (message) => {
  throw new Error(`[issue-20] ${message}`);
};
...
const checkPrivateBoundary = () => {
  ...
  equal(imports, ["../issue-17/oracle.mjs"], "Dice oracle imports anything beyond the issue-17 public oracle boundary");
  assert(!/(?:packages|src|dist)\//.test(source), "Dice oracle references production package/source/dist paths");
  assert(path.join(directory, "oracle.mjs").includes(`${path.sep}verification${path.sep}issue-20${path.sep}`), "Dice oracle escaped its private verification directory");
};
...
console.log(`[issue-20] Dice semantic profile, independent oracle, ${golden.cases.length} literal cases, all-side grid, and private-boundary checks passed`);
