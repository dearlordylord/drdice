 ...
 * generic over every Unicode astral code point.  The bounded BMP fast path
 * only avoids rechecking the reviewed non-ASCII scanner mutations; all other
 * BMP and lone-surrogate literals use the same generic fallback. */
...
type HighMatchInBlock<C extends string, Block extends readonly string[], LowBlocks extends readonly (readonly string[])[] = LowSurrogateBlocks> = Block extends readonly [infer Head extends string, ...infer Tail extends string[]]
  ? LowMatch<C, Head, LowBlocks> extends infer Found extends string
    ...
    : never
  : never;
/* The reviewed BMP mutations take a constant-size one-unit path.  Every other
 * literal is classified by the generic surrogate-pair lookup; a valid pair
 * also recovers its exact high unit for diagnostics. */
type FastBmpUnit = "\u00a0" | "\u2003";
type Utf16Units<C extends string> = string extends C ? [unknown] : C extends AsciiUnit ? [unknown]
  : C extends FastBmpUnit ? [unknown] : [HighMatch<C>] extends [never] ? [unknown] : [unknown, unknown];
type Utf16FirstUnit<C extends string> = string extends C ? C : C extends AsciiUnit ? C
  : C extends FastBmpUnit ? C : [HighMatch<C>] extends [never] ? C : HighMatch<C>;
